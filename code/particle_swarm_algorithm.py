...
class PSOConfig:
    def __init__(self, size=20, pw=0.5, gw=0.5, w=0.5, v_max=4.0, stag_lim=10):
        self.size = size
...
def pso(cfg=None):
    ...
    while iteration <= 500 or max(cost_list) >= 0:
        ...
        for p_idx, p in enumerate(swarm_list):
            ...
            for vect_idx in range(len(p.solution)):
                ...
                new_pos = (p.solution[vect_idx] + p.vel[vect_idx]) % 5
        ...
        new_fitness_list = swarm_cost(swarm_list)
        if max(new_fitness_list) <= max(cost_list):
            stagnation += 1
        else:
            stagnation = 0
    ...
    return g_best_pos, iteration, regens
...
def is_diagonal(row1, col1, row2, col2):
    if row1 == row2 and col1 == col2:  # Don't count same position as diagonal
        return False

    if abs(row1 - row2) == abs(col1 - col2):
        return True  # Return True if 'pos1' diagonal to 'pos2'
    else:
        return False
...
def display_board(queen_list: list):
    board = []
    for i in range(8):
        board.append([0, 0, 0, 0, 0, 0, 0, 0])

    for row, col in enumerate(queen_list):
        board[7 - row][col - 1] = 'X'
...
if __name__ == "__main__":
    DEBUG = 0
    pso_cfg = PSOConfig(size=3, pw=0.5, gw=0.5, w=0.5)
    solution, iterations, regenerations = pso(pso_cfg)
    print(
        f"Found solution: {solution} after {iterations} iterations ({regenerations} regenerations)")
